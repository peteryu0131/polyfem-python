#!/bin/bash
# =============================================================================
# Slurm: theta-degree Batch 3 (theta_batch3_cases / sharded runner).
#
# Default: 3 array tasks (shards), each running 244/3≈81 cases with up to 5
# parallel PolyFEM **processes** per node.
#
# Submit from **polyfem-python repo root**::
#   mkdir -p slurm_logs
#   sbatch scripts/slurm/sbatch_run_theta_batch3.sh
#
# Force re-run::
#   sbatch --export=ALL,EXTRA_PY_ARGS='--force' scripts/slurm/sbatch_run_theta_batch3.sh
# =============================================================================

#SBATCH --job-name=theta_b3

#SBATCH --output=slurm_logs/slurm-%A_%a-theta_b3.out
#SBATCH --error=slurm_logs/slurm-%A_%a-theta_b3.err
#SBATCH --array=0-2
...
PY_SCRIPT="experiment/new_experiment/theta_degree/run_theta_batch3_sharded.py"
if [[ ! -f "$PY_SCRIPT" ]]; then
