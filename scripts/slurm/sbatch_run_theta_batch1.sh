#!/bin/bash
# =============================================================================
# Slurm: theta-degree Batch 1 (theta_batch1_cases / sharded runner).
#
# Default: 16 array tasks (shards), each running 1600/16=100 cases with up to 5
# parallel PolyFEM **processes** per node.
#
# Submit from **polyfem-python repo root**::
#   mkdir -p slurm_logs
#   sbatch scripts/slurm/sbatch_run_theta_batch1.sh
#
# Force re-run (ignore success rows in each shard CSV)::
#   sbatch --export=ALL,EXTRA_PY_ARGS='--force' scripts/slurm/sbatch_run_theta_batch1.sh
# =============================================================================

#SBATCH --job-name=theta_b1

#SBATCH --output=slurm_logs/slurm-%A_%a-theta_b1.out
#SBATCH --error=slurm_logs/slurm-%A_%a-theta_b1.err
#SBATCH --array=0-15
...
PY_SCRIPT="experiment/new_experiment/theta_degree/run_theta_batch1_sharded.py"
if [[ ! -f "$PY_SCRIPT" ]]; then
