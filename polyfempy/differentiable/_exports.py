"""Public and compatibility export lists for ``polyfempy.differentiable``."""

from __future__ import annotations


PUBLIC_API = [
    "prepare_differentiable_simulation",
    "DifferentiableResult",
    "DifferentiableMaterialResult",
    "ParameterizedVertexDesign",
    "ObjectiveLossResult",
    "make_von_mises_loss",
    "make_stress_norm_loss",
    "make_optimizer",
    "OptimizationKind",
    "OptimizationProblem",
    "OptimizationRunResult",
    "prepare_optimization_problem",
    "prepare_parameterized_shape_problem",
    "run_optimization",
    "body_vertex_mask",
    "shape_gradient_for_body",
    "save_training_sample",
]

ADVANCED_COMPAT_API = [
    "solve_differentiable",
    "solve_differentiable_material_from_youngs",
    "build_solver_from_settings",
    "solver_body_ids_for_assembly",
    "solver_body_slot_mask",
    "solve_differentiable_material",
    "youngs_value_to_internal",
    "youngs_to_lame",
    "build_lame_from_youngs",
    "make_bounds_projector",
    "make_named_parameter_map",
    "make_parameter",
    "relative_scale",
    "scale_selected_vertices",
    "scale_selected_vertices_about_axis_center",
    "scale_selected_vertices_about_x_center",
    "selected_axis_center",
    "selected_x_center",
    "tan_half_angle_scale",
    "vertices_axis_le",
    "vertices_y_le",
    "create_polyfem_objective",
    "make_material_von_mises_loss",
    "material_design_vector",
    "SmoothTimeAggregationName",
    "TimeAggregation",
    "TimeAggregationName",
    "ScalarMaterialOptimizationProblem",
    "ScalarMaterialOptimizationStep",
    "apply_finite_difference_gradient_fallback",
    "collect_material_chain_diagnostics",
    "evaluate_material_loss_value_for_E",
    "finite_difference_material_E_gradient",
    "format_optional",
    "format_scalar_material_optimization_history_summary",
    "format_scalar_material_optimization_step",
    "make_material_optimizer",
    "objective_solution_rhs_diagnostics",
    "print_material_chain_diagnostics",
    "print_objective_rhs_diagnostics",
    "prepare_optimization_baseline_simulation",
    "prepare_material_differentiable_simulation",
    "prepare_material_optimization_problem",
    "prepare_scalar_youngs_material_problem",
    "report_optimization_baseline",
    "run_scalar_material_optimization",
    "usable_scalar_gradient",
    "ParameterizedShapeOptimizationProblem",
    "ShapeOptimizationProblem",
    "ShapeOptimizationStep",
    "format_shape_optimization_history_summary",
    "format_shape_optimization_step",
    "make_shape_optimizer",
    "make_von_mises_shape_loss",
    "prepare_parameterized_shape_differentiable_simulation",
    "prepare_parameterized_shape_optimization_problem",
    "prepare_shape_differentiable_simulation",
    "prepare_shape_optimization_problem",
    "print_shape_optimization_step",
    "run_shape_optimization",
    "get_direct_von_mises_monitor",
    "gradient_norm",
    "print_loss_summary",
    "print_parameterized_shape_summary",
    "print_scalar_material_summary",
]

ADVANCED_API = ADVANCED_COMPAT_API
COMPATIBILITY_API = ADVANCED_COMPAT_API

EXPORT_MODULES = {
    # Recommended public API.
    "prepare_differentiable_simulation": ".runtime.solve",
    "DifferentiableResult": ".runtime.result",
    "DifferentiableMaterialResult": ".runtime.result",
    "ParameterizedVertexDesign": ".design",
    "ObjectiveLossResult": ".objectives.bridge",
    "make_von_mises_loss": ".objectives.bridge",
    "make_stress_norm_loss": ".objectives.bridge",
    "make_optimizer": ".optimization.problem",
    "OptimizationKind": ".optimization.problem",
    "OptimizationProblem": ".optimization.problem",
    "OptimizationRunResult": ".optimization.problem",
    "prepare_optimization_baseline_simulation": ".optimization.problem",
    "prepare_optimization_problem": ".optimization.problem",
    "prepare_parameterized_shape_problem": ".shape.optimization",
    "report_optimization_baseline": ".optimization.problem",
    "run_optimization": ".optimization.problem",
    "body_vertex_mask": ".shape.mask",
    "shape_gradient_for_body": ".shape.mask",
    "save_training_sample": ".data.training",
    # Explicit compatibility / advanced imports. These stay available, but are
    # loaded lazily by ``polyfempy.differentiable`` so the top-level facade stays
    # small.
    "solve_differentiable": ".runtime.solve",
    "solve_differentiable_material_from_youngs": ".runtime.solve",
    "build_solver_from_settings": ".runtime.settings",
    "solver_body_ids_for_assembly": ".material.parameters",
    "solver_body_slot_mask": ".material.parameters",
    "solve_differentiable_material": ".runtime.solve",
    "youngs_value_to_internal": ".material.parameters",
    "youngs_to_lame": ".material.parameters",
    "build_lame_from_youngs": ".material.parameters",
    "make_bounds_projector": ".design",
    "make_named_parameter_map": ".design",
    "make_parameter": ".design",
    "relative_scale": ".shape.geometry_maps",
    "scale_selected_vertices": ".shape.geometry_maps",
    "scale_selected_vertices_about_axis_center": ".shape.geometry_maps",
    "scale_selected_vertices_about_x_center": ".shape.geometry_maps",
    "selected_axis_center": ".shape.geometry_maps",
    "selected_x_center": ".shape.geometry_maps",
    "tan_half_angle_scale": ".shape.geometry_maps",
    "vertices_axis_le": ".shape.geometry_maps",
    "vertices_y_le": ".shape.geometry_maps",
    "create_polyfem_objective": ".objectives.bridge",
    "make_material_von_mises_loss": ".objectives.bridge",
    "material_design_vector": ".objectives.bridge",
    "SmoothTimeAggregationName": ".objectives.bridge",
    "TimeAggregation": ".objectives.bridge",
    "TimeAggregationName": ".objectives.bridge",
    "ScalarMaterialOptimizationProblem": ".material.optimization",
    "ScalarMaterialOptimizationStep": ".material.optimization",
    "format_scalar_material_optimization_history_summary": ".material.optimization",
    "format_scalar_material_optimization_step": ".material.optimization",
    "make_material_optimizer": ".material.optimization",
    "prepare_material_differentiable_simulation": ".material.optimization",
    "prepare_material_optimization_problem": ".material.optimization",
    "prepare_scalar_youngs_material_problem": ".material.optimization",
    "run_scalar_material_optimization": ".material.optimization",
    "ParameterizedShapeOptimizationProblem": ".shape.optimization",
    "ShapeOptimizationProblem": ".shape.optimization",
    "ShapeOptimizationStep": ".shape.optimization",
    "format_shape_optimization_history_summary": ".shape.optimization",
    "format_shape_optimization_step": ".shape.optimization",
    "make_shape_optimizer": ".shape.optimization",
    "make_von_mises_shape_loss": ".shape.optimization",
    "prepare_parameterized_shape_differentiable_simulation": ".shape.optimization",
    "prepare_parameterized_shape_optimization_problem": ".shape.optimization",
    "prepare_shape_differentiable_simulation": ".shape.optimization",
    "prepare_shape_optimization_problem": ".shape.optimization",
    "print_shape_optimization_step": ".shape.optimization",
    "run_shape_optimization": ".shape.optimization",
    "get_direct_von_mises_monitor": ".objectives.bridge",
    "gradient_norm": ".optimization.summary",
    "print_loss_summary": ".optimization.summary",
    "print_parameterized_shape_summary": ".optimization.summary",
    "print_scalar_material_summary": ".optimization.summary",
    "apply_finite_difference_gradient_fallback": ".material.diagnostics",
    "collect_material_chain_diagnostics": ".material.diagnostics",
    "evaluate_material_loss_value_for_E": ".material.diagnostics",
    "finite_difference_material_E_gradient": ".material.diagnostics",
    "format_optional": ".material.diagnostics",
    "objective_solution_rhs_diagnostics": ".material.diagnostics",
    "print_material_chain_diagnostics": ".material.diagnostics",
    "print_objective_rhs_diagnostics": ".material.diagnostics",
    "usable_scalar_gradient": ".material.diagnostics",
}

__all__ = [
    "PUBLIC_API",
    "ADVANCED_COMPAT_API",
    "ADVANCED_API",
    "COMPATIBILITY_API",
    "EXPORT_MODULES",
]
