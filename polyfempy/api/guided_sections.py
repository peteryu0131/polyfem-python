"""Public guided section support, presets, and config translation.

This module contains the preset helpers and ``build_config(...)`` implementation
behind ``polyfempy.api.guided``.  It is kept inside the ``polyfempy`` package so
installed users do not depend on the repository-local ``experiment`` tree.
"""
